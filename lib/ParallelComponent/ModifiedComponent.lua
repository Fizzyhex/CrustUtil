...
function Component.new(config: ComponentConfig)
	...
	customComponent.Tag = config.Tag or "<NO TAG>"
	...
	customComponent.ParallelInstance = config.ParallelInstance and config.ParallelInstance.Value
...
function Component:_setup()
	...
	if self.ParallelInstance then
		StartWatchingInstance(self.ParallelInstance)
		
		self[KEY_TROVE]:Connect(CollectionService:GetInstanceRemovedSignal(self.Tag), function(instance)
			if instance == self.ParallelInstance then
				InstanceUntagged(self.ParallelInstance)
			end
		end)
